...
[#function getFieldCoffeeType type]
	[#if type.enum]
		[#local fieldType = "type : 'string', pattern : /${enumConstants[type.name]}/"]
	[#elseif type.collection]
		[#local fieldType = "type : 'array', items : { ${getFieldCoffeeType(type.typeParameters[0])} }"]
...
${clazz.name?uncap_first}Schema =
	...
	[#list clazz.superClassesFields as field]
		${field.initialName} : { ${getFieldCoffeeType(field.type)}, ${getIsRequired(field)} }
	[/#list]
	[#list clazz.fields as field]
		${field.initialName} : { ${getFieldCoffeeType(field.type)}, ${getIsRequired(field)} [#if field.required && field.type.collection], minItems: 1[/#if] }
	[/#list]
[/#if]
